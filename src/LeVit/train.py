...
parser.add_argument('--dataname', metavar = 'NAME', default = '', help='data name')
...
parser.add_argument('--lr-k-decay', type=float, default=0.5,
...
parser.add_argument('--aux_logits', default=False, help='aux logit')
...
def main():
    ...
    if args.model_ema:
        ...
        if args.resume:
            load_checkpoint(model_ema.module, args.resume, use_ema=True, strict=False)
    ...
    if args.dataname == 'CALTECH101':
        path_caltech101 = args.data_dir + '101_ObjectCategories/'
        dataset_total =  torchvision.datasets.ImageFolder(path_caltech101, transform=transform_train)
        train_size = int(0.8 * len(dataset_total))
        test_size = len(dataset_total) - train_size
        dataset_train, dataset_eval = torch.utils.data.random_split(dataset_total, [train_size, test_size])
    if args.dataname == 'STL10':
